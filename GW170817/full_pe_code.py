...
nodes_gauss_num = 0.1
size = 10000000
best_match = [1.4574227, 1.2993017, -0.018811436, 0.011776949]
bounds = [[best_match[0] - 0.1, best_match[0] + 0.1], [best_match[1] - 0.1, best_match[1] + 0.1], [best_match[2] - 0.1, best_match[2] + 0.1], [best_match[3] - 0.1, best_match[3] + 0.1]]
# ------- Optimization ----------
def neg_net_snr(params):
    return  - net_snr(params, data = data, psd = psd)

fid_params = optimization(best_match, neg_net_snr, bounds = bounds)

# -------- Defining Interpolation Boundaries -----------
